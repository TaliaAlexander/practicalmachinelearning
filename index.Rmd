...
```{r}
train_raw <- read.csv("C:/Users/ruf6/Downloads/pml-training.csv")
test_raw <- read.csv("C:/Users/ruf6/Downloads/pml-testing.csv")
...
We will remove unnecessary and mostly-NA variables.
...
We will split the original training data set into a new training set (70%) and a validation set (30%). The test set will be left out until the end when we apply the models fit using the testing and validation sets.
...
The new training set has 13,737 observations and the validation set has 5,885 observations, each with 53 variables. We will use 4-fold cross validation.
...
Now we will predict the classe outcome on the 20 cases in the test set using the best performing model: Random Forest, with 99.37% accuracy.
